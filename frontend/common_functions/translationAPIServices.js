...
const recognizeAndTranslate = async (audioFile, destLang) => {
  const formData = new FormData();
  formData.append('file', audioFile);
  formData.append('dest_lang', destLang);

  try {
    const response = await fetch('/api/recognizeAndTranslate', {
      method: 'POST',
      body: formData,
    });
    if (!response.ok) {
...
export { translateText, getOptions, getVoiceForText, recognizeAndTranslate }